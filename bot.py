...
class DM_settings(commands.Cog):
    ...
    async def dm_add(
        ...
        find_command = cursor.execute(f"""SELECT command, enabled FROM user_commands WHERE command = (?) AND discord_user = (?)""", (command, user)).fetchone()
...
async def add_command(ctx, user, command):
    cursor.execute(
        f"""INSERT INTO user_commands (discord_user, command, enabled) VALUES (?, ?, ?)""", (user, command, 1)
    )
    connection.commit()
...
async def toggle_on(ctx, user, command):
    cursor.execute(
        """ UPDATE user_commands SET enabled = (?) WHERE discord_user = (?) AND command = (?)""", (1, user, command)
    )
    connection.commit()
...
async def toggle_off(ctx, user, command):
    cursor.execute(
        """ UPDATE user_commands SET enabled = (?) WHERE discord_user = (?) AND command = (?)""", (0, user, command)
    )
    connection.commit()
...
#channel.history(after=message) use for channel

#asyncio.sleep() use for pausing script

#Add message.author.user functionality into logic for loop
...
async def is_user_command(message):
    ...
    user_commands = cursor.execute(
        """SELECT command FROM user_commands WHERE discord_user = (?) AND enabled = (?)""", (user, 1)
    ).fetchall()
...
async def create_alert_embed(command, guild, message, value, link):
    embed = discord.Embed(title='Command Alert', description=f'You just used `{command[0]}` in {guild.name}', color=0x0000)
    embed.set_thumbnail(url="https://raw.githubusercontent.com/yaboiwierdal/dm_bot/main/images/circle-cropped(4).png")
    embed.add_field(name="Message", value=f"{message.content}", inline=False)
    embed.add_field(name="Bot's Reply", value=f"{value}", inline=False)
    embed.add_field(name="Conversation", value=f"[Jump to message!]({link})", inline=False)
